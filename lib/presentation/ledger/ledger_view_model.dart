...
class LedgerViewModel extends GetxController with ScrollMixin {
  ...
  void _getCategories() async {
    isFiltering = false;
    try {
      await _categoryUseCases.getUseCase.get("ledger").then((categories) {
  ...
  void _getLedgers(int page, {bool? queryMore = false}) async {
    try {
      await _ledgerUseCases.getUseCase.get(page).then((response) {
  ...
  void _getLedgersByFilter(int page, {bool? queryMore = false}) async {
    isFiltering = true;
    final filter = Filter(
      categoryId:
  ...
  void _filterLedger() {
    ...
    _page = 0;
    update();
    _getLedgersByFilter(_page);
  }

  void deleteLedger(Entry ledger) async {
    _loading.show();
    showInterstitialAd(
      loading: _loading,
      notHidden: true,
  ...
  void navigateToLedgerForm(Entry ledger) async {
    printInfo(info: "ledger => ${ledger.entryRemark}");
    Get.to(
      () => LedgerFormPage(
  ...
  void _reloadAction() {
    ...
    _page = 0;
    update();
    isFiltering ? _getLedgersByFilter(_page) : _getCategories();
  }
  ...
  Future<void> onEndScroll() async {
    if (isEnableQueryMore) {
      ...
      isFiltering
          ? _getLedgersByFilter(_page, queryMore: true)
